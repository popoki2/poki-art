...
// 🛒 [v1.4.0] 고가치 상점 아이템 데이터베이스 (인플레이션 방지 및 성취감 보장)
const SHOP_ITEMS = {
    badges: [
    ...
    colors: [
        { id: 'color_blue',    name: '🔵 시원한 블루', price: 20000, hex: '#1e90ff' },
        { id: 'color_green',   name: '🟢 네온 그린',   price: 20000, hex: '#00ff7f' },
        { id: 'color_purple',  name: '🟣 영롱한 퍼플', price: 45000, hex: '#a855f7' },
        { id: 'color_orange',  name: '🟧 불꽃 오렌지', price: 45000, hex: '#ff6b00' },
        { id: 'color_gold',    name: '✨ 럭셔리 골드', price: 100000, hex: '#ffd700' },
        { id: 'color_pink',    name: '💖 포키 핫핑크', price: 300000, hex: '#ff007f' }
    ]
};
...
io.on('connection', (socket) => {
    ...
    socket.on('register', ({ username, password, nickname }) => {
        ...
        usersDB[username] = {
            ...
            points: 10000, // 신규 가입 축하 지원금
            inventory: [],
            equipped: { badge: '🔰', color: '#000000' }
        };
        saveUsersDB(usersDB);
    ...
    socket.on('buyItem', ({ category, itemId }) => {
        ...
        if (user.inventory && user.inventory.includes(itemId)) {
            return socket.emit('shopResponse', { success: false, message: '이미 보유 중인 상품입니다.' });
        }

        if (user.points < item.price) {
            return socket.emit('shopResponse', { 
                success: false, 
                message: `포인트가 부족합니다! (필요: ${item.price.toLocaleString()} Pt)` 
            });
        }
        ...
        socket.emit('shopResponse', { 
            success: true, 
            message: `'${item.name}' 구매 및 장착 완료!`,
            user 
        });
        io.emit('updateLobbyUsers', Object.values(lobbyUsers));
    });
    ...
    function joinRoomAction(socket, roomId, password, currentUser) {
        ...
        const player = {
            ...
            color: user.equipped?.color || '#000000'
        };
    ...
    socket.on('chatMessage', (msg) => {
        ...
        if (room.isPlaying && !isDrawer && !isAlreadySolved && msg.trim() === room.currentWord) {
            ...
            const earnedScore = Math.max(1500 - (solveOrder - 1) * 200, 500); // 점수 수급량 증가
...
// ==========================================
// [v1.3.1 배포 에러 완벽 해결 반영] HTML Render 함수
// ==========================================
...
<head>
    <meta charset="UTF-8">
    <title>Poki Party v1.4.0 - 포키파티</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'MapleStory', sans-serif, sans-serif; }
        ...
        /* 인게임 레이아웃: CSS Grid 정밀 계산으로 채팅창 뚫림 전면 수정 */
        #game-room.active { 
            display: grid; 
        ...
        .modal-content { background: #fff; padding: 20px; border-radius: 12px; border: 4px solid #f48fb1; width: 360px; display: flex; flex-direction: column; gap: 10px; }
...
<body>
    ...
    <div id="auth-screen" class="screen active">
        ...
        <p style="color: #888; font-size: 13px;">v1.4.0 최고의 퀄리티로 즐기는 그림 퀴즈 파티</p>
    ...
    <div id="loading-screen" class="screen">
        ...
        <h2 style="color: #d81b60; font-size: 26px;">🎉 포키파티에 접속 중입니다... 🎉</h2>
    </div>
    ...
    <div id="lobby-screen" class="screen">
        <div class="lobby-side">
            ...
            <button class="btn" style="background:#ff9800;" onclick="openShopModal()">🛒 상점 방문</button>
        </div>
    ...
    <div id="create-modal" class="modal">
        <div class="modal-content">
            ...
            <select id="room-category">
                ...
                <option value="lol">메이플/하스/롤</option>
            </select>
    ...
    <div id="shop-modal" class="modal">
        <div class="modal-content" style="width: 420px; max-height: 550px;">
            <h3 style="color: #d81b60;">🛒 v1.4.0 포인트 상점</h3>
            <div style="display:flex; gap:10px; margin-bottom:10px;">
                <button class="btn" onclick="renderShopCategory('badges')">뱃지 목록</button>
                <button class="btn" style="background:#9c27b0;" onclick="renderShopCategory('colors')">닉네임 색상</button>
            </div>
            <div id="shop-items-container" style="flex:1; overflow-y:auto; display:flex; flex-direction:column; gap:8px;"></div>
            <div style="display: flex; justify-content: flex-end; margin-top: 10px;">
                <button class="btn" style="background:#ccc;" onclick="closeShopModal()">닫기</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function updateProfileUI(user) {
            ...
            const color = (user && user.equipped && user.equipped.color) ? user.equipped.color : '#000000';
            ...
            document.getElementById('user-profile-info').innerHTML = 
                '<b>' + badge + ' <span style="color:' + color + '">' + nickname + '</span></b> (' + username + ')<br>' +
        ...
        function renderShopCategory(cat) {
            ...
            SHOP_ITEMS_CLIENT[cat].forEach(item => {
                ...
                div.style.cssText = 'background:#fff; border:1px solid #f8bbd0; padding:8px; border-radius:6px; display:flex; justify-content:space-between; align-items:center;';
                div.innerHTML = 
                    '<div>' +
                        '<b>' + item.name + '</b><br>' +
                        '<small style="color:#777;">' + (item.desc || '') + '</small><br>' +
                        '<b style="color:#d81b60;">💰 ' + item.price.toLocaleString() + ' Pt</b>' +
                    '</div>';
...
server.listen(PORT, () => {
    console.log(`=================================================`);
    console.log(` Poki Party v1.4.0 Engine Loaded Successfully!`);
